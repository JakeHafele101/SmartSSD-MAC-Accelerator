...
namespace ML {
    ...
    class Layer {
        ...
        public:
            ...
            Layer(const LayerParams inParams, const LayerParams outParams, LayerType lType)
                : inParams(inParams), outParams(outParams), outData(outParams), lType(lType), aType(ActivationType::NONE), perf(false) {}
            Layer(const LayerParams inParams, const LayerParams outParams, LayerType lType, ActivationType aType)
            : inParams(inParams), outParams(outParams), outData(outParams), lType(lType), aType(aType), perf(false) {}
            Layer(const LayerParams inParams, const LayerParams outParams, LayerType lType, ActivationType aType, bool perf)
            : inParams(inParams), outParams(outParams), outData(outParams), lType(lType), aType(aType), perf(perf) {}
            ...
            bool getPerf() const { return perf; }
    ...
    float LayerData::compare(const LayerData& other) const {
        LayerParams aParams = this->getParams();
        ...
        if (aParams.dims.size() != bParams.dims.size()) {
            std::cerr << "LayerData arrays must have the same number of dimentions ("
                        << aParams.dims.size() << " and " << bParams.dims.size() << ")\n";
        }
