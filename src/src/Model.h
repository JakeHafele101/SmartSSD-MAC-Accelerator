...
namespace ML {
    class Model {
        public:
            // Constructors
            Model() : layers(), perf(false) {} //, checkFinal(true), checkEachLayer(false) {}
            Model(bool perf) : layers(), perf(perf) {} //, checkFinal(true), checkEachLayer(false) {}
            ...
            void testLayers(std::filesystem::path basePath, std::string imgname);
